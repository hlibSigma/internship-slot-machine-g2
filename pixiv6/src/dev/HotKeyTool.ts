export default class HotKeyTool {
    ...
    public showAllPopupWithHotKeysDescriptions():void {
        ...
        for (const actions of Array.from(this.actions.values())) for (const action of actions) {
    ...
    public showPopupWithActionsDescription(actions:Array<Action>):boolean {
        let message:string = "Actual reserved hot keys:\n";
    ...
    public registerOnKeyCode(keyCode:number, action:Function, description:string = "Empty description"):void {
        const key = this.getKeyCode(keyCode);
        if (!this.actions.has(key)) {
    ...
    public registerOnKey(key:string, action:Function, description:string = "Empty description"):void {
        ...
        if (theSameAction != null && actions) {
            ...
            let startIndex = actions.indexOf(theSameAction);
